  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        className="group relative flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl bg-slate-100/90 dark:bg-slate-900/90 hover:bg-slate-200 dark:hover:bg-slate-800 border border-slate-300 dark:border-slate-700/80 text-slate-800 dark:text-slate-200 transition-all duration-200 cursor-pointer shadow-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40"
        aria-label="Select Theme Mode"
        aria-expanded={isOpen}
      >
        {/* Animated Palette / Active Icon */}
        <div className="relative flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-slate-200 dark:bg-slate-800 text-cyan-600 dark:text-cyan-400 group-hover:scale-110 transition-transform">
          <CurrentIcon className="w-3.5 h-3.5" style={{ color: currentOption.accentColor }} />
        </div>

        <div className="flex items-center gap-1 sm:gap-1.5 font-mono text-[11px] sm:text-xs font-semibold tracking-wider">
          <span>{currentOption.name}</span>
          <ChevronDown className={`w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-64 max-w-[calc(100vw-2rem)] rounded-2xl bg-white/95 dark:bg-[#0c1222]/95 backdrop-blur-2xl border border-slate-200/90 dark:border-slate-800 shadow-2xl shadow-slate-900/20 dark:shadow-[0_15px_40px_rgba(0,0,0,0.7)] p-2 z-50 overflow-hidden"
          >
            {/* Header Title */}
